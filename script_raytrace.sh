#!/usr/bin/env bash
#
# script_raytrace.sh - setup, baseline, profiling, optimized run and
# comparison for the pyperformance "raytrace" benchmark.
#
# Usage:
#   ./script_raytrace.sh setup       install tools, copy the original benchmark
#   ./script_raytrace.sh baseline    pyperformance run of the original benchmark
#   ./script_raytrace.sh profile     cProfile, perf report and flame graph (original)
#   ./script_raytrace.sh optimized   pyperformance run of the optimized benchmark
#   ./script_raytrace.sh profile-opt cProfile, perf report and flame graph (optimized)
#   ./script_raytrace.sh compare     compare baseline and optimized results
#   ./script_raytrace.sh save NAME   snapshot the current optimized results
#   ./script_raytrace.sh all         every stage above, in order
#
# Settings can be overridden from the environment, for example:
#   MODE=--rigorous ./script_raytrace.sh baseline
#   PROFILE_LOOPS=10 ./script_raytrace.sh profile
#   SKIP_APT=1 ./script_raytrace.sh setup      (packages already installed)
#
# Repository layout used by this script:
#   script_raytrace.sh
#   raytrace/bm_raytrace/        original benchmark (copied from pyperformance)
#   raytrace/bm_raytrace_opt/    optimized benchmark (edit run_benchmark.py here)
#   raytrace/MANIFEST_opt       pyperformance manifest for the optimized copy
#   raytrace/bench_driver.py    runs bench_pyflake() directly for profiling
#   raytrace/results/           all generated output
...
BENCH_ROOT="$ROOT/raytrace"
ORIG_DIR="$BENCH_ROOT/bm_raytrace"
OPT_DIR="$BENCH_ROOT/bm_raytrace_opt"
...
# $BENCH_ROOT (add raytrace/venv/ to .gitignore).
...
stage_setup() {
    ...
    local src
    src="$("$PYTHON" -c 'import pyperformance, os; print(os.path.dirname(pyperformance.__file__))')/data-files/benchmarks/bm_raytrace"
    [[ -d "$src" ]] || die "bm_raytrace not found in $src"
    mkdir -p "$BENCH_ROOT"
    if [[ ! -d "$ORIG_DIR" ]]; then
        cp -r "$src" "$ORIG_DIR"
    ...
    if [[ ! -d "$OPT_DIR" ]]; then
        ...
        # The json result keeps the name "raytrace" (set in run_benchmark.py),
        # so baseline and optimized results can be compared directly.
        cat > "$OPT_DIR/pyproject.toml" <<'EOF'
[project]
name = "pyperformance_bm_raytrace_opt"
requires-python = ">=3.8"
dependencies = ["pyperf"]
version = "1.0.0"
...
stage_baseline() {
    ...
    pyperformance_cmd run -p "$(python_path)" -b raytrace $MODE \
...
stage_optimized() {
    ...
    pyperformance_cmd run -p "$(python_path)" --manifest "$OPT_MANIFEST" \
        -b raytrace_opt $MODE --same-loops "$RESULTS/baseline.json" \
...
profile_one() {
    ...
    "$FLAMEGRAPH_DIR/flamegraph.pl" --title "raytrace ($label) - $PYTHON_DBG" \
    ...
    if [[ -n "$pyspy" ]]; then
        ...
        if [[ -s "$RESULTS/pyspy_$label.folded" ]]; then
            ...
            "$PYTHON" "$ROOT/raytrace/summarize_pyspy.py" \
