...
# A run in the first RLE stage is four identical bytes followed by a count.
run_re = re.compile(rb"(.)\1{3}", re.DOTALL)
...
class Bitfield(BitfieldBase):
    ...
    def skipbits(self, n):
        # Consume n bits that snoopbits() has already returned.
        self.bits -= n
        self.bitfield >>= n
...
class RBitfield(BitfieldBase):
    ...
    def _fill(self, n):
        # Inlined refill: avoids needbits() -> _more() -> _read() per byte.
    ...
    def snoopbits(self, n=8):
        ...
        if n > bits:
            self._fill(n)
            bits = self.bits
        return (self.bitfield >> (bits - n)) & ((1 << n) - 1)
    ...
    def readbits(self, n=8):
        ...
        bitfield = self.bitfield
        r = bitfield >> bits
        self.bitfield = bitfield & ((1 << bits) - 1)
        self.bits = bits
        return r
...
class HuffmanTable(object):
    ...
    def build_lookup(self):
        # Canonical codes of one length form a contiguous numeric range, so a
        # table indexed by the next max_bits bits decodes a symbol directly.
        tbl = self.table
        maxb = self.max_bits
        symbols = [-1] * (1 << maxb)
        lengths = [0] * (1 << maxb)
        for x in tbl:
            n = x.bits
            shift = maxb - n
            start = x.symbol << shift
            span = 1 << shift
            symbols[start:start + span] = [x.code] * span
            lengths[start:start + span] = [n] * span
        self.lookup_symbols = symbols
        self.lookup_lengths = lengths
    ...
    def find_next_symbol(self, field, reversed=True):
        ...
        for x in self.table:
            ...
            if cached_length != bits:
                cached = snoopbits(bits)
                cached_length = bits
            if (x.reverse_symbol if reversed else x.symbol) == cached:
                field.skipbits(bits)
                return x.code
...
def bwt_transform(L):
    # One bucket of positions per byte value, filled in a single pass and then
    # concatenated. This replaces sorting the whole block, the 256 find()
    # scans and the indexed writes of the original implementation.
    buckets = [[] for _ in range(256)]
    appenders = [bucket.append for bucket in buckets]
    for i, symbol in enumerate(L):
        appenders[symbol](i)

    pointers = []
    extend = pointers.extend
...
def decode_huffman_block(b, out):
    ...
    buffer = []
    t = None
    t_symbols = t_lengths = None
    t_max = 0
    snoopbits = b.snoopbits
    skipbits = b.skipbits
    while True:
        decoded -= 1
        if decoded <= 0:
            ...
            if selector_pointer <= len(selectors_list):
                ...
                selector_pointer += 1
                t_symbols = t.lookup_symbols
                t_lengths = t.lookup_lengths
                t_max = t.max_bits

        peek = snoopbits(t_max)
        r = t_symbols[peek]
        skipbits(t_lengths[peek])
        if 0 <= r <= 1:
            if repeat == 0:
                repeat_power = 1
    ...
    # Pointless/irritating run-length encoding step. The runs are located with
    # a regular expression so that the scan between runs runs in native code
    # instead of one Python loop iteration per byte.
    n = len(nt)
    last = n - 5          # a run needs four bytes plus a count byte
    search = run_re.search
    append = out.append
    pos = 0
    while pos < n:
        m = search(nt, pos)
        if m is None or m.start() > last:
            append(nt[pos:])
            break
        start = m.start()
        if start > pos:
            append(nt[pos:start])
        append(nt[start:start + 1] * (nt[start + 4] + 4))
        pos = start + 5
